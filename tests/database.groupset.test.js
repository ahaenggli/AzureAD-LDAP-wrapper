...
${'LDAP_USERS_SETDEFAULTGROUP'}  | ${'secu|groupofothergroups'}              | ${true}     | ${''}| ${'2'}
`('test database.js with $EnvName=$EnvVal', ({ EnvName, EnvVal, validResult, envSuffix, syncTime }) => {
    ...
    test('callback should be called', async () => {
        ...
        expect(database.getEntries()).toHaveProperty('uid=lynner,cn=users,dc=domain,dc=tld');
        expect(database.getEntries()).toHaveProperty('uid=pradeepg,cn=users,dc=domain,dc=tld');
        expect(database.getEntries()).toHaveProperty('cn=secu,cn=groups,dc=domain,dc=tld');
        expect(database.getEntries()).toHaveProperty('cn=users,cn=groups,dc=domain,dc=tld');

        const secu = database.getEntries()['cn=secu,cn=groups,dc=domain,dc=tld'];
        const usr = database.getEntries()['cn=users,cn=groups,dc=domain,dc=tld'];
        const lynner = database.getEntries()['uid=lynner,cn=users,dc=domain,dc=tld'];
        const pradeepg = database.getEntries()['uid=pradeepg,cn=users,dc=domain,dc=tld'];

        expect(lynner.gidNumber).toBe(usr.gidNumber);
        expect(pradeepg.gidNumber).toBe(secu.gidNumber);
